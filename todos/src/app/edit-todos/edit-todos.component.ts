...
export class EditTodosComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(!this.dataPoint.editIndex.showData){
     ...
     this.newEditData = this.reactiveForm.group(this.dataPoint.listOfTodos[this.data?.index]) 
    }
  }
  ...
  saveEdit(){
    ...
    this.dataPoint.listOfTodos[this.data.index] = JSON.parse(JSON.stringify(this.newEditData.value));
    this.dataSave=true
    setTimeout(()=>{
      this.router.navigate(['list'])
    },3000)
